    ...
    return (
        <div className='my-5 border rounded p-4 '>
            <div className="flex items-center">
                <div className="border mr-3 h-10 w-10 rounded-full bg-[gray] overflow-hidden">
                    <img src={require('../assets/user.jpg')} height={"100%"} width={'100%'} />
                </div>
                <div>{e?.user?.name || "bot"}</div>
            </div>
            {
                <div className="flex justify-between">
                    <div className='font-bold text-xl mt-3' >{e?.title || ''}</div>
            ...
            {e?.image && <img className="sm:max-w-[300px] max-w-[100px] " src={e?.image} onError={({ currentTarget }) => {
